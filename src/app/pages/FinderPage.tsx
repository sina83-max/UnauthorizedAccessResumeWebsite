...
export function FinderPage({ isDarkMode, onToggleDark }: { isDarkMode: boolean; onToggleDark: () => void }) {
  ...
  const [showPrefsDenied, setShowPrefsDenied] = useState(false);
  ...
  return (
    <div className={`min-h-screen w-full select-none font-sans overflow-hidden transition-colors duration-500 ${isDarkMode ? 'dark text-slate-100' : 'text-slate-900'}`}>
      
      {/* RENDER CONNECT SCREEN OR DESKTOP ENVIRONMENT */}
      <AnimatePresence mode="wait">
        {!isConnected ? (
          /* =======================================================================
             ENTRY SCREEN - macOS Connect to Server Modal
             ======================================================================= */
          <motion.div
            key="connect-modal"
          ...
          /* =======================================================================
             DESKTOP ENVIRONMENT & FINDER WORKSPACE
             ======================================================================= */
          <motion.div
            key="desktop"
            initial={{ opacity: 0 }}
          ...
          <div className="flex items-center gap-4">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button className="flex items-center gap-1.5 font-bold hover:bg-white/10 dark:hover:bg-white/5 rounded px-1.5 py-0.5 -ml-1.5 transition-colors">
                  <svg className="w-4 h-4" viewBox="0 0 170 170" fill="currentColor">
                    <path d="M150.37 130.25c-2.45 5.66-5.35 10.87-8.71 15.66-4.58 6.53-8.33 11.05-11.22 13.56-4.48 4.12-9.28 6.23-14.42 6.35-3.69 0-8.14-1.05-13.32-3.18-5.2-2.12-9.97-3.17-14.34-3.17-4.58 0-9.49 1.05-14.75 3.17-5.28 2.13-9.54 3.24-12.8 3.35-4.93.21-9.84-1.96-14.75-6.52-3.13-2.73-7.05-7.41-11.76-14.04-5.05-7.1-9.2-15.33-12.46-24.71-3.51-10.12-5.27-19.95-5.27-29.5 0-10.95 2.36-20.4 7.09-28.31 3.72-6.34 8.64-11.33 14.79-14.97 6.15-3.64 12.79-5.5 19.94-5.64 3.92 0 9.06 1.21 15.43 3.59 6.36 2.39 10.44 3.6 12.24 3.6 1.34 0 5.87-1.42 13.57-4.24 7.29-2.62 13.44-3.71 18.46-3.28 13.64 1.1 23.88 6.48 30.68 16.17-12.19 7.39-18.2 17.74-18.07 31.01.12 10.34 3.86 18.95 11.2 25.8 3.34 3.17 7.07 5.62 11.22 7.36-.9 2.61-1.85 5.11-2.86 7.51zM119.11 7.24c0 8.1-2.96 15.67-8.86 22.67-7.12 8.32-15.73 13.13-25.07 12.37a25.2 25.2 0 0 1-.19-3.07c0-7.78 3.39-16.1 9.4-22.9 3-3.44 6.82-6.31 11.45-8.6 4.62-2.26 8.99-3.51 13.1-3.71.12 1.1.17 2.2.17 3.24z" />
                  </svg>
                </button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="start" sideOffset={4} className="w-56">
                <DropdownMenuItem onClick={() => setShowAboutMac(true)} className="font-semibold">
                  <Info className="w-4 h-4 mr-2" />
                  About This Mac
                </DropdownMenuItem>
                <DropdownMenuSeparator />
                <DropdownMenuItem onClick={() => setShowPrefsDenied(true)}>
                  <Settings className="w-4 h-4 mr-2" />
                  System Preferences...
                </DropdownMenuItem>
                <DropdownMenuSeparator />
                <DropdownMenuItem onClick={() => onToggleDark()}>
                  {isDarkMode ? <Sun className="w-4 h-4 mr-2" /> : <MoonIcon className="w-4 h-4 mr-2" />}
            ...
            {/* System Name (not clickable) */}
            <span className="font-bold">Sina's Remote Server</span>

            {/* Menu Bar Items (non-clickable, hover only) */}
            <div className="hidden sm:flex items-center gap-3 text-muted-foreground">
              <span className="font-semibold text-foreground">Finder</span>
              <span className="hover:text-foreground hover:bg-white/10 dark:hover:bg-white/5 rounded px-1.5 py-0.5 cursor-default transition-colors">File</span>
              <span className="hover:text-foreground hover:bg-white/10 dark:hover:bg-white/5 rounded px-1.5 py-0.5 cursor-default transition-colors">Edit</span>
              <span className="hover:text-foreground hover:bg-white/10 dark:hover:bg-white/5 rounded px-1.5 py-0.5 cursor-default transition-colors">View</span>
              <span className="hover:text-foreground hover:bg-white/10 dark:hover:bg-white/5 rounded px-1.5 py-0.5 cursor-default transition-colors">Go</span>
              <span className="hover:text-foreground hover:bg-white/10 dark:hover:bg-white/5 rounded px-1.5 py-0.5 cursor-default transition-colors">Window</span>
              <span className="hover:text-foreground hover:bg-white/10 dark:hover:bg-white/5 rounded px-1.5 py-0.5 cursor-default transition-colors">Help</span>
            </div>
          </div>
              ...
              <div className="flex-1 flex overflow-hidden">
                ...
                <aside className="w-48 bg-secondary/30 border-r border-border/40 p-3 hidden sm:flex flex-col gap-6 text-xs select-none">
                  ...
                  <div className="mt-auto p-3 rounded-xl bg-card/60 border border-border/40 text-[11px] text-muted-foreground space-y-1">
                    ...
                    <p>Select any file and press <kbd className="inline-flex items-center justify-center px-4 py-0.5 bg-secondary border border-border/80 rounded-md font-mono text-[10px] shadow-sm"><span className="w-4 h-0.5 rounded-full bg-muted-foreground/50" /></kbd> for Quick Look preview.</p>
                  </div>
                </aside>
        ...
        {quickLookFile && (
          <motion.div
            ...
            className={`fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md
              ${quickLookFile.type === 'pdf' ? 'p-0 sm:p-4' : 'p-4'}
            `}
          ...
          >
            <motion.div
              ...
              className={`
                w-full bg-card/90 backdrop-blur-2xl border border-white/20 dark:border-white/10 rounded-2xl shadow-2xl flex flex-col overflow-hidden
            ...
            >
              ...
              {quickLookFile.type === 'pdf' ? (
                <div className="flex-1 min-h-0 overflow-hidden p-0 sm:p-0 bg-background/50">
                  <div className="h-full flex flex-col">
                    <div className="flex items-center justify-between px-3 sm:px-4 py-2 border-b border-border bg-background/80 backdrop-blur shrink-0">
                      ...
                      <a
                        ...
                        download={quickLookFile.downloadName || 'resume.pdf'}
                        className="px-3 py-1.5 rounded-xl bg-primary text-primary-foreground text-xs font-semibold flex items-center gap-1.5 shadow hover:bg-primary/90 transition-all"
                      >
                    ...
                    <iframe
                      src={quickLookFile.content || '/uploads/resume.pdf'}
              ...
              <div className="flex-1 overflow-y-auto p-6 bg-background/50">
                ...
                {quickLookFile.type === 'vcf' && (
                  <div className="max-w-md mx-auto p-6 bg-card rounded-2xl border border-border shadow-xl flex flex-col items-center text-center gap-4">
                    <div className="w-20 h-20 rounded-full bg-gradient-to-tr from-blue-500 to-violet-600 flex items-center justify-center text-white text-2xl font-bold shadow-lg">
                      SB
                    </div>
                    <div>
                      <h3 className="text-lg font-bold">Sina Beignazari</h3>
                      <p className="text-xs text-muted-foreground">Backend Dev and Data Enthusiast</p>
                    </div>
                    ...
                    <div className="w-full space-y-2 text-xs text-left pt-2 border-t border-border">
                      <div className="flex items-center gap-3 p-2 rounded-xl bg-secondary/50">
                        ...
                        <span className="font-mono">s.beignazary1383@gmail.com</span>
                      </div>
                      <div className="flex items-center gap-3 p-2 rounded-xl bg-secondary/50">
                        <Phone className="w-4 h-4 text-blue-500" />
                        <span className="font-mono">+98 933 828 5859</span>
                      </div>
                      <div className="flex items-center gap-3 p-2 rounded-xl bg-secondary/50">
                        <MapPin className="w-4 h-4 text-blue-500" />
                        <span>Koohak Blvd, District 22, Tehran</span>
                      </div>
                      <a href="https://github.com/sina83-max" target="_blank" rel="noopener noreferrer"
                        ...
                        <span className="font-mono text-primary">github.com/sina83-max</span>
                      </a>
                      <a href="https://linkedin.com/in/sina-beignazari" target="_blank" rel="noopener noreferrer"
                         className="flex items-center gap-3 p-2 rounded-xl bg-secondary/50 hover:bg-secondary transition-colors">
                        <Linkedin className="w-4 h-4 text-blue-500" />
                        <span className="font-mono text-primary">linkedin.com/in/sina-beignazari</span>
                      </a>
                    </div>

                    <a
                      href={`data:text/vcard;charset=utf-8,${encodeURIComponent(quickLookFile.content || '')}`}
                      download="Sina_Beignazari.vcf"
                      className="w-full py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-semibold text-xs flex items-center justify-center gap-2 shadow-lg shadow-blue-600/20 transition-all"
                    >
        ...
        {showAboutMac && (
          ...
          >
            <motion.div
              ...
              onClick={(e) => e.stopPropagation()}
              className="w-full max-w-lg bg-card/95 backdrop-blur-2xl border border-white/20 dark:border-white/10 rounded-2xl shadow-2xl overflow-hidden"
            >
              {/* Title Bar */}
              <div className="h-10 border-b border-border/40 px-4 flex items-center justify-between bg-secondary/30 select-none">
                <div className="flex items-center gap-2 group">
                  <button
                    onClick={() => setShowAboutMac(false)}
                    className="w-3 h-3 rounded-full bg-rose-500 hover:bg-rose-600 transition-colors flex items-center justify-center text-[8px] text-rose-950 font-bold"
                ...
                <span className="text-xs font-semibold text-muted-foreground">About This Mac</span>
                <div className="w-16" />
              </div>

              {/* Content */}
              <div className="p-8 flex flex-col items-center text-center gap-4">
                {/* MacBook Pro SVG Illustration */}
                <svg className="w-32 h-20 mb-1" viewBox="0 0 200 130" fill="none">
                  {/* Screen */}
                  <rect x="20" y="5" width="160" height="95" rx="8" fill="currentColor" className="text-slate-800" />
                  <rect x="26" y="11" width="148" height="79" rx="4" fill="currentColor" className="text-slate-700" />
                  <rect x="26" y="11" width="148" height="79" rx="4" fill="url(#screenGlow)" opacity="0.3" />
                  {/* Apple logo on screen */}
                  <path d="M97 40c0 3.5-1.3 6.8-3.8 9.8-2.6 3-5.5 4.6-7.6 4.7 0.1-0.6 0.1-1.2 0.1-1.9 0-3.2 1.4-6.6 3.9-9.3 1.3-1.4 2.8-2.6 4.6-3.5 1.8-0.9 3.5-1.5 4.9-1.6-0.1 0.5-0.1 1-0.1 1.8z" fill="currentColor" className="text-slate-400" opacity="0.6" />
                  {/* Base */}
                  <path d="M10 105 C10 102, 12 100, 16 100 L184 100 C188 100, 190 102, 190 105 L195 120 C195 124, 192 127, 188 127 L12 127 C8 127, 5 124, 5 120 Z" fill="currentColor" className="text-slate-500" />
                  <path d="M16 100 L184 100 L188 105 L12 105 Z" fill="currentColor" className="text-slate-400" />
                  {/* Touch Bar notch */}
                  <rect x="75" y="100" width="50" height="3" rx="1.5" fill="currentColor" className="text-slate-600" />
                  <defs>
                    <linearGradient id="screenGlow" x1="100" y1="11" x2="100" y2="90" gradientUnits="userSpaceOnUse">
                      <stop stopColor="#0a84ff" stopOpacity="0.4" />
                ...
                <div className="w-full grid grid-cols-2 gap-3 text-left">
                  <div className="p-3 rounded-xl bg-secondary/50 border border-border/40">
                    <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground mb-1">Chip</p>
                    <p className="text-xs font-semibold text-foreground">Apple M4 Max</p>
                    <p className="text-[10px] text-muted-foreground">16-core CPU · 40-core GPU</p>
                  </div>
                  <div className="p-3 rounded-xl bg-secondary/50 border border-border/40">
                    <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground mb-1">Memory</p>
                    <p className="text-xs font-semibold text-foreground">48 GB</p>
                    <p className="text-[10px] text-muted-foreground">Unified Memory</p>
                  </div>
                  <div className="p-3 rounded-xl bg-secondary/50 border border-border/40">
                    <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground mb-1">Display</p>
                    <p className="text-xs font-semibold text-foreground">16.2" Liquid Retina XDR</p>
                    <p className="text-[10px] text-muted-foreground">3456 × 2234 · ProMotion</p>
                  </div>
                  <div className="p-3 rounded-xl bg-secondary/50 border border-border/40">
                    <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground mb-1">Storage</p>
                    <p className="text-xs font-semibold text-foreground">1 TB SSD</p>
                    <p className="text-[10px] text-muted-foreground">Up to 7.4 GB/s read</p>
                  </div>
                  <div className="p-3 rounded-xl bg-secondary/50 border border-border/40">
                    <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground mb-1">Serial Number</p>
                    <p className="text-xs font-semibold text-foreground font-mono">C02XK1YZJHD3</p>
                  </div>
                  <div className="p-3 rounded-xl bg-secondary/50 border border-border/40">
                    <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground mb-1">Processor</p>
                    <p className="text-xs font-semibold text-foreground">14-core CPU</p>
                    <p className="text-[10px] text-muted-foreground">10P + 4E cores</p>
                  </div>
                </div>

                {/* Additional Info */}
                <div className="w-full p-3 rounded-xl bg-secondary/30 border border-border/30 text-[10px] text-muted-foreground space-y-1">
                  <div className="flex justify-between">
                    <span>Model Identifier</span>
                    <span className="font-mono text-foreground">Mac16,1</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Model Number</span>
                    <span className="font-mono text-foreground">MYW83LL/A</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Warranty</span>
                    <span className="text-emerald-500 font-semibold">AppleCare+ Active</span>
                  </div>
                  <div className="flex justify-between">
      ...
      {/* =======================================================================
         SYSTEM PREFERENCES ACCESS DENIED MODAL
         ======================================================================= */}
      <AnimatePresence>
        {showPrefsDenied && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm"
            onClick={() => setShowPrefsDenied(false)}
          >
            <motion.div
              initial={{ scale: 0.95 }}
              animate={{ scale: 1 }}
              exit={{ scale: 0.95 }}
              onClick={(e) => e.stopPropagation()}
              className="w-full max-w-sm bg-card/95 backdrop-blur-2xl border border-white/20 dark:border-white/10 rounded-2xl shadow-2xl p-6 flex flex-col items-center text-center gap-4"
            >
              ...
              <div>
                <h3 className="text-base font-bold text-foreground">Access Denied</h3>
                <p className="text-xs text-muted-foreground mt-1.5 leading-relaxed">
                  System Preferences is restricted. You do not have permission to modify system settings on this remote machine.
                </p>
              </div>
              <button
                onClick={() => setShowPrefsDenied(false)}
